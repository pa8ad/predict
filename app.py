...
def contest_profiles() -> Dict[str, ContestProfile]:
    return {
        "CQ WPX CW": ContestProfile(
            name="CQ WPX CW",
            mult_label="Prefixes",
            description="WPX prefixes per band; CQ WPX rules reward each new prefix heavily versus modest continent-based QSO points.",
            default_weights={
                "mult": 16.0,
                "band_mult": 8.0,
                "fresh": 3.0,
        ...
        "CQ WW DX CW": ContestProfile(
            ...
            description=(
                "Country multipliers (per band) with zone/country scoring; CQ WW rules make both run rate and high-value new "
                "countries critical."
            ),
            default_weights={
                "mult": 12.0,
        ...
        "IARU HF": ContestProfile(
            ...
            mult_label="HQ/ITU",
            description=(
                "IARU zones/HQ per band; points scale by continent (1/3/5) and HQ are 5 pts, so balance run productivity with "
                "fresh multipliers."
            ),
            default_weights={
                "mult": 11.0,
